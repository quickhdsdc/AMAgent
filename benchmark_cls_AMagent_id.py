...
ML_TRAIN_OVERRIDE = {
    # OOD exp   -> use this experiment's TRAIN split for the ML block
    "Exp_OOD_1": "Exp_OOD_2",
    "Exp_OOD_2": "Exp_OOD_3",
    "Exp_OOD_3": "Exp_OOD_4",
    "Exp_OOD_4": "Exp_OOD_1",
...
def _load_partial_results(stem: str) -> pd.DataFrame:
    out_path = f"./results_AM/AMagent_id/gpt5_preds_{stem}.csv"
    cols = [
        "row_idx",
        "material",
        "Power",
...
def _train_ml_and_predict_proba(
    train_stem: str,            # <--- CHANGED: model selection keyed by the *actual* train stem
    df_train: pd.DataFrame,
    df_test: pd.DataFrame
) -> Dict[int, Dict[str, float]]:
    model_name = BEST_MODELS.get(train_stem, "RF")   # <--- CHANGED
    num_feats = ["Power", "Velocity", "beam D", "layer thickness"]
    cat_feats = ["material"]

    # (unchanged preprocessing & training code below)
    # ------------------------------------------------
    # Build X/y
    cols_needed = [c for c in num_feats + cat_feats if c in df_train.columns]
    if LABEL_COL not in df_train.columns:
        raise RuntimeError(f"{train_stem}: '{LABEL_COL}' not found in train set.")
    ...
    if model_name == "GB":
        ...
        clf = RandomForestClassifier(
            n_estimators=300, max_depth=None, random_state=randomState, n_jobs=-1
        )
    ...
    if len(X_train) == 0:
        raise RuntimeError(f"{train_stem}: No valid training rows after label normalization.")
    ...
    for ridx, idx in enumerate(df_test.index):
        ...
        proba_by_row[int(idx)] = {k: round(row_map[k], 4) for k in LABEL_ORDER}
    return proba_by_row
...
def evaluate_gpt5_on_experiment_with_resume(
    stem: str,
    gpt_profile: str = "default",
) -> Dict[str, float]:
    # 1) Load current test split (unchanged)
    ...
    # 2) Choose the *training* split for the ML block
    #    If an override exists, we use that train split to build an in-distribution model.
    train_stem_for_ml = ML_TRAIN_OVERRIDE.get(stem, stem)  # <--- NEW
    df_train_for_ml = _load_exp_train(train_stem_for_ml)    # <--- NEW

    # Describe the ML model provenance for the prompt
    exp_type_for_prompt = "in-distribution" if stem in ML_TRAIN_OVERRIDE else _exp_type_from_stem(stem)

    # 3) Train ML (on the chosen train stem) & get per-row probabilities
    ml_probs_by_row: Dict[int, Dict[str, float]] = {}
    if df_train_for_ml is not None:
        try:
            ml_probs_by_row = _train_ml_and_predict_proba(
                train_stem=train_stem_for_ml,     # <--- CHANGED
                df_train=df_train_for_ml,
                df_test=df_test
            )
        except Exception as e:
            print(f"[WARN] ML training failed for {stem} (train={train_stem_for_ml}): {e}")

    # 4) Load partial results (unchanged)
    df_partial = _load_partial_results(stem)
    done_row_idxs = set(df_partial["row_idx"].tolist())

    # 5) Query LLM for missing rows (only change is passing exp_type_for_prompt)
    ...
    df_join["pred_label_canon"] = df_join["gpt_pred_label"].astype(str).str.lower()
    ...
    return {
        ...
        "n_scored": n_scored,
        "n_completed_rows": len(_load_partial_results(stem)),
    }
...
def main():
    ...
    for stem in EXPERIMENTS:
        print("\n=======================================")
        print(f"AM Agent evaluation on {stem} (resumable)")
        print("=======================================")
    ...
    print(f"\nWrote AM Agent summary: {out_csv}")

if __name__ == "__main__":
    # main()

    SEARCH_PATTERNS = [
        "./results_AM/AMagent_id/gpt5_preds_*.csv"
    ]

    OUT_CSV = "./results_AM/AMagent_id/metrics_summary.csv"
