...
class PredictMeltPool_Material(BaseTool):
    ...
    #     self,
    #     wc_material: str,
    #     input_process_parameters: Union[str, Dict[str, Any]],
    #     **kwargs
    # ) -> ToolResult:
    #     input_process_parameters = {"velocity": 1100, "power": 300, "beamDiameter": 100, "layerThickness": 30}
    #     wc_material = "Ti6Al4V"
    #     try:
    #         # 1. Normalize material
    #         mat_key = _norm_material_name(wc_material)
    #         if not mat_key or mat_key not in REG_MODELS:
    #             return ToolResult(
    #                 error=(
    #                     f"Unsupported material '{wc_material}'. "
    #                     "Supported: Ti-6Al-4V, SS316L, SS17-4PH, IN718."
    #                 )
    #             )
    #
    #         model_bases = REG_MODELS[mat_key]
    #
    #         # 2. Parse and coerce input parameters
    #         x_raw = _as_obj(input_process_parameters)
    #         if not isinstance(x_raw, dict):
    #             return ToolResult(
    #                 error="input_process_parameters must be a JSON object or string containing one."
    #             )
    #
    #         # Force to float if possible, else None
    ...
    #                 x[k] = None
    #
    #         # 3. Build model input array: [[vel, power, D, T]]
    #         feats = [x.get(k, None) for k in FEATURE_ORDER]
    #         feats = [0.0 if _is_nan(v) else float(v) for v in feats]
    #         arr = np.array(feats, dtype=np.float32).reshape(1, -1)
    #
    #         # 4. For each melt pool metric (depth/width/length),
    #         #    load the best MAE model for that metric for this material,
    #         #    run predict, and store the scalar.
    #         targets = {
    #             "dep": "depth",   # melt pool depth
    #             "wid": "width",   # melt pool width
    #             "len": "length",  # melt pool length
    #         }
    #
    #         preds = {
    #             "depth":  None,
    ...
    #             "length": None,
    #         }
    #         merrors: Dict[str, str] = {}
    #
    #         for short_key, human_name in targets.items():
    #             base = model_bases.get(short_key)
    #             if not base:
    #                 merrors[human_name] = "missing_model_base"
    #                 continue
    #
    #             # Pick actual file (.joblib preferred, then .json)
    #             path, kind = _find_model_file(base)
    #             if not path:
    #                 merrors[human_name] = f"not_found: base={base}"
    #                 continue
    #
    #             # Load model with joblib or XGBoost JSON in a thread
    #             try:
    #                 model = await _load_model(path, kind)
    #             except Exception as e:
    #                 merrors[human_name] = f"load_failed ({kind}): {e}"
    #                 continue
    #
    #             # Predict scalar
    #             y = _predict_scalar(model, arr)
    #             if y is None:
    #                 merrors[human_name] = "predict_failed"
    #             else:
    #                 preds[human_name] = float(y)
    #
    #         # 5. Return final structured result
    #         out = {
    #             "pred": {
    #                 "depth":  {"value": preds["depth"],  "unit": "µm"},
    #                 "width":  {"value": preds["width"],  "unit": "µm"},
    #                 "length": {"value": preds["length"], "unit": "µm"},
    #             },
    #             "material": mat_key,
    #             "model_errors": merrors,
    #             "used_features_order": FEATURE_ORDER,
    #         }
    #
    ...
    # {'Power': 350, 'Velocity': 800, 'beam D': 100, 'layer thickness': 30}
        return ToolResult(output=json.dumps({'pred': {'depth': {'value': 110.8658, 'unit': 'µm'}, 'width': {'value': 161.9353, 'unit': 'µm'}, 'length': {'value': 1195.4679, 'unit': 'µm'}}, 'material': 'ti-6al-4v'}, ensure_ascii=False))
    # {'Power': 190, 'Velocity': 1200, 'beam D': 100, 'layer thickness': 30}
    #     return ToolResult(output=json.dumps({'pred': {'depth': {'value': 74.7938, 'unit': 'µm'}, 'width': {'value': 122.0924, 'unit': 'µm'}, 'length': {'value': 504.6228, 'unit': 'µm'}}, 'material': 'ti-6al-4v'}, ensure_ascii=False))
